...
- **QA docs** — paste a link (a Claude artifact, a Google Doc, whatever you write them in) into a PR's ⋯ or a category's ⋯. It shows as a 🧪 QA chip on the row or in the category header, tinted to the category's colour, and opens in a tab. Give the doc a name and the name shows on hover, otherwise it's the URL. Put it on the category to cover a whole epic, or on a single PR for one ticket.
